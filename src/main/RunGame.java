...
public class RunGame
{
	...
	public RunGame(String args[])
	{
		...
		this.timeout = Integer.parseInt(args[6]);
	}
	...
	private void go() throws IOException, InterruptedException
	{
		...
		//start the engine
		this.engine = new Engine(map, player1, player2, timeout);
	...
	public void saveGame(IORobot bot1, IORobot bot2) {
		...
		try {
			dump = new PrintWriter(this.dumpDir + "/player/player_io.txt");
			dump.println(bot1.getDump());
		} catch(Exception e){
			e.printStackTrace();
		} finally {
		...
		try {
			dump = new PrintWriter(this.dumpDir + "/player/player_err.txt");
			dump.println(bot1.getStderr());
		} catch(Exception e){
			e.printStackTrace();
		} finally {
			if(dump != null) dump.close();
		...
		try {
			dump = new PrintWriter(this.dumpDir + "/opponent/opponent_io.txt");
			dump.println(bot2.getDump());
		} catch(Exception e){
			e.printStackTrace();
		} finally {
			if(dump != null) dump.close();
		...
		try {
			dump = new PrintWriter(this.dumpDir + "/opponent/opponent_err.txt");
			dump.println(bot2.getStderr());
		} catch(Exception e){
			e.printStackTrace();
		} finally {
			if(dump != null) dump.close();
